...
const App = {
    ...
    async init() {
        ...
        // --- Mostrar botón Marcador si está habilitado ---
        if (user.attendance_marker_enabled) {
            const navMarker = document.getElementById('nav-marker');
            if (navMarker) navMarker.style.display = 'flex';
        }

        // --- Role-based UI Adjustments & Initial View ---
        if (user.role === 'employee') {
            const allowedViews = ['calculator'];
            if (user.attendance_marker_enabled) allowedViews.push('marker');
    ...
    startMarkerCamera: async (video) => {
        try {
            ...
            if (container) {
                container.innerHTML = `<div style="display:flex; align-items:center; justify-content:center; width:100%; height:100%; color:var(--text-muted); text-align:center; padding:1rem; font-size:0.85rem;">
                    <div><i data-lucide="camera-off" style="width:32px; height:32px; margin-bottom:8px; display:block; margin:0 auto 8px;"></i>Cámara no disponible.<br>Verifique los permisos del navegador.</div>
                </div>`;
    ...
    renderLogin() {
        ...
        if (registerLink) {
            registerLink.onclick = (e) => {
                e.preventDefault();
                window.open('https://pay.hotmart.com/L104170872R', '_blank');
            };
        }
...
const Views = {
    ...
    marker: async () => {
        ...
        // Obtener logs de hoy para mostrar estado
        const logs = await Storage.get('logs');
        const today = Storage.getLocalDate();
        const todayLogs = logs.filter(l => l.date && l.date.split('T')[0] === today);

        const todayByEmp = {};
        todayLogs.forEach(l => {
            const eid = l.employee_id;
            if (!todayByEmp[eid]) todayByEmp[eid] = [];
            todayByEmp[eid].push(l);
        });

        let todayStatusHtml = '';
        if (isAdmin && todayLogs.length > 0) {
            const statusRows = activeEmployees.map(e => {
                const empLogs = todayByEmp[e.id] || [];
                if (empLogs.length === 0) return null;
                const lastLog = empLogs[empLogs.length - 1];
                const hasOpen = lastLog.time_in && !lastLog.time_out;
                const status = hasOpen
                    ? `<span style="color: var(--success); font-weight: 600;">Trabajando (desde ${lastLog.time_in})</span>`
                    : `<span style="color: var(--text-muted);">Completado (${empLogs.length} registro${empLogs.length > 1 ? 's' : ''})</span>`;
                return `<tr><td style="font-weight:600;">${e.name}</td><td>${status}</td></tr>`;
            }).filter(Boolean).join('');

            if (statusRows) {
                todayStatusHtml = `
                    <div class="card-container" style="margin-top: 2rem;">
                        <h3 style="margin-bottom: 1rem; color: var(--primary);"><i data-lucide="activity" style="width:18px; margin-right:8px; vertical-align:middle"></i> Estado de Hoy</h3>
                        <div class="table-container">
                            <table>
                                <thead><tr><th>Empleado</th><th>Estado</th></tr></thead>
        ...
        return `
            <div class="card-container" style="border: 1px solid var(--primary); background: rgba(99, 102, 241, 0.03);">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
                    <h3 style="margin: 0; color: var(--primary)"><i data-lucide="map-pin" style="width:20px; margin-right:8px; vertical-align:middle"></i> Marcador de Asistencia</h3>
                ...
                <div id="marker-controls" style="display: flex; flex-direction: column; gap: 15px; align-items: center;">
                    ...
                    ${user.attendance_photo_required ? `
                        ...
                        <button class="btn btn-secondary" id="btn-snap" style="width: 100%; max-width: 320px;"><i data-lucide="camera" style="width:18px; margin-right:8px"></i> Activar Cámara</button>
                    ` : ''}
    ...
    init_marker: async () => {
        ...
        if (user.attendance_photo_required && video) {
            ...
            if (btnSnap) {
                btnSnap.onclick = async () => {
                    if (!cameraStarted) {
                        await App.startMarkerCamera(video);
                        cameraStarted = true;
                        btnSnap.textContent = '📷 Tomar Selfie';
                        return;
                    }
                    capturedPhoto = App.takeMarkerPhoto(video);
        ...
        if (btnIn) btnIn.onclick = () => handleMarkerAction('IN');
        if (btnOut) btnOut.onclick = () => handleMarkerAction('OUT');
    },
